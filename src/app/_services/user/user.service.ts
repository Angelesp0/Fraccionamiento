...
export class UserService {
  ...
  postPayment(id, description, amount, status, update_time, type, users_id_users) {
      return this.http.post('http://192.168.100.35:3000/payments/' + id, {description, amount, status, update_time, type, users_id_users}, this.httpOptions);
  }
  ...
  postReceipt(id, date, file, value, payments_id_payments) {
    ...
    return this.http
      .post('http://192.168.100.35:3000/users/' + id + '/receipt', uploadData)
      .pipe(
        catchError(this.handleError)
      );
  }
